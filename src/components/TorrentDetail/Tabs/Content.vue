...
export default {
  ...
  methods: {
    ...
    renameFile(item) {
      qbit.renameFile(this.hash, item.name, item.newName).catch(() => Vue.$toast.error(this.$t('toast.renameFileFailed')))

      item.name = item.newName
      this.toggleEditing(item)
    },
    renameFolder(item) {
      qbit.renameFolder(this.hash, item.name, item.newName).catch(() => Vue.$toast.error(this.$t('toast.renameFolderFailed')))
