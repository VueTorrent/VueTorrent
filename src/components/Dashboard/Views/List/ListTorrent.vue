...
const stateColor = computed(() => current.value.colors[`torrent-${getTorrentStateColor(props.torrent.state)}`])
</script>
...
<template>
  <v-card
    ...
    width="100%"
    :color="isTorrentSelected ? `torrent-${getTorrentStateColor(torrent.state)}-darken-3` : undefined"
