...
export default class Torrent {
  ...
  constructor(data: QbitTorrent, format = 'DD/MM/YYYY, HH:mm:ss') {
    ...
    this.completed_on = data.completion_on > 0 ? dayjs(data.completion_on * 1000).format(format) : t('dashboard.not_complete')
    ...
    this.seen_complete = data.seen_complete > 0 ? dayjs(data.seen_complete * 1000).format(format) : t('dashboard.not_complete').toString()
